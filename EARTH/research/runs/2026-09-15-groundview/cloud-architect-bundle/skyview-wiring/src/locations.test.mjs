...
test('typed WGS84 pairs are coordinates, not address strings', () => {
  const kc = parseWgs84Query('39.0997, -94.5786');
  assert.equal(kc.lat, 39.0997);
  assert.equal(kc.lon, -94.5786);

  const gis = parseWgs84Query('-94.5786, 39.0997');
  assert.equal(gis.lat, 39.0997);
  assert.equal(gis.lon, -94.5786);

  const compass = parseWgs84Query('39.0997 N, 94.5786 W');
  assert.equal(compass.lat, 39.0997);
  assert.equal(compass.lon, -94.5786);

  const labeled = parseWgs84Query('lat 39.0997 lon -94.5786');
  assert.equal(labeled.lat, 39.0997);
  assert.equal(labeled.lon, -94.5786);

  assert.equal(parseWgs84Query('Kansas City'), null);
  assert.equal(parseWgs84Query('1600 Pennsylvania'), null);
  assert.equal(parseWgs84Query('39.0997'), null);
});

test('a coordinate search flies to the point and never geocodes', async () => {
  const viewer = stubViewer();
  let geocodeCalls = 0;
  const destination = await searchAndFlyTo(viewer, '39.0997, -94.5786', {
    placeSearch: {
      async geocode() {
        geocodeCalls += 1;
        throw new Error('geocoder must not see a WGS84 pair');
      },
    },
  });
  assert.equal(geocodeCalls, 0);
  assert.equal(destination.navigationMode, 'close-coordinate');
  assert.equal(destination.label, '39.0997, -94.5786');
  assert.equal(destination.rangeM, 12000);
  assert.equal(viewer.flights.length, 1);
});
