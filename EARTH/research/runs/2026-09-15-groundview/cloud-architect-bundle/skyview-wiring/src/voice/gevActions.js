...
function focusDataLayerRow(layerId) {
  const row = document.querySelector(`#data-toggles [data-layer-id="${CSS.escape(layerId)}"], #skyview-toggles [data-layer-id="${CSS.escape(layerId)}"]`);
  if (!row) return null;
...
function layerTitle(layerId) {
  ...
  if (layerId === 'groundview') return 'GROUNDVIEW';
