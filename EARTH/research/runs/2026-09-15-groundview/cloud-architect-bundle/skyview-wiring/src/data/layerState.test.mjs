...
test('production registry is exact, canonical, and rejects incomplete contracts', async () => {
  ...
  assert.ok(REGISTERED_LAYER_IDS.includes('groundview'));
  assert.ok(!REGISTERED_LAYER_IDS.includes('local-pipeline-offices'));
  assert.equal(LAYER_STATE_REGISTRY.find((entry) => entry.id === 'groundview')?.token, 'k');
  assert.equal(new Set(LAYER_STATE_REGISTRY.map((entry) => entry.token)).size, 17);
  assert.throws(
