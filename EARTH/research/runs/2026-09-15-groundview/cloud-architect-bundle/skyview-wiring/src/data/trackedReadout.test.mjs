...
test('selection lifecycle ignores vessels, accepts installations, and clears without stale cards', () => {
  ...
  try {
    ...
    fakeWindow.dispatchEvent(new CustomEvent('gev:entity-selected', {
      detail: { layerId: 'groundview', entity: {
        gevTrackedId: 'groundview:receiver:GV-RX-01',
        gevDisplayPosition: () => ({ x: 4, y: 5, z: 6 }),
        gevLabelModel: { title: 'GV-RX-01', details: ['OBSERVED receiver site'], accent: '#7fd4b8' },
      } },
    }));
    assert.equal(getActiveTrackedReadoutId(), 'groundview:receiver:GV-RX-01');
