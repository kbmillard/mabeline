...
/** Range for a typed WGS84 pair: neighborhood, not a rooftop. */
const COORDINATE_RANGE_M = 12000;

const WGS84_NUMBER = String.raw`[+\u2212-]?\d+(?:\.\d+)?`;

function signedDegrees(raw) {
  const text = String(raw || '').replace(/\u2212/g, '-');
  const value = Number(text);
  return Number.isFinite(value) ? value : null;
}

function packWgs84(lat, lon) {
...
/**
 * Parse a LOCATION-box query that is already a WGS84 point.
 * Place-name geocoders treat "39.0997, -94.5786" as an address string and can
 * fly to an unrelated sandy hit; coordinates never go through that path.
 * First number is latitude unless it cannot be (abs > 90), then lon,lat.
 */
export function parseWgs84Query(query) {
  const text = String(query || '').trim();
  if (!text) return null;

  const labeled = text.match(new RegExp(
    `^lat(?:itude)?\\s*[:=]?\\s*(${WGS84_NUMBER})\\s*[,\\s]+lon(?:g(?:itude)?)?\\s*[:=]?\\s*(${WGS84_NUMBER})$`,
    'i',
  ));
  if (labeled) return packWgs84(signedDegrees(labeled[1]), signedDegrees(labeled[2]));

  const compass = text.match(new RegExp(
    `^(${WGS84_NUMBER})\\s*[°º]?\\s*([NSns])\\s*[,\\s]+\\s*(${WGS84_NUMBER})\\s*[°º]?\\s*([EWew])$`,
  ));
  if (compass) {
    let lat = Math.abs(signedDegrees(compass[1]));
    let lon = Math.abs(signedDegrees(compass[3]));
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
    if (compass[2].toUpperCase() === 'S') lat = -lat;
    if (compass[4].toUpperCase() === 'W') lon = -lon;
    return packWgs84(lat, lon);
  }

  const pair = text.match(new RegExp(
    `^(${WGS84_NUMBER})\\s*[°º]?\\s*[,;\\s]\\s*(${WGS84_NUMBER})\\s*[°º]?$`,
  ));
  if (!pair) return null;
  const a = signedDegrees(pair[1]);
  const b = signedDegrees(pair[2]);
  if (Math.abs(a) > 90 && Math.abs(a) <= 180 && Math.abs(b) <= 90) return packWgs84(b, a);
  return packWgs84(a, b);
}
 ...
 * A WGS84 pair skips geocoding entirely.
 */
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  const parsed = parseWgs84Query(query);
  let lat;
  let lng;
  let label;
  let types;
  let viewport;
  const coordinateQuery = Boolean(parsed);
  ...
  const navigationMode = coordinateQuery ? 'close-coordinate' : geocodeNavigationMode(types);
  ...
  const shouldResolveBuilding = !coordinateQuery && navigationMode === 'precise-place';
  ...
  const flight = flyToLandmark(viewer, buildingBounds?.lat ?? lat, buildingBounds?.lon ?? lng, {
    range,
    pitch: coordinateQuery ? -45 : buildingPitch(buildingBounds),
    heading: coordinateQuery ? 0 : 30,
    buildingHeight: coordinateQuery ? 0 : 30,
    buildingBounds,
    duration,
    onStart: options.onStart,
    onComplete: options.onComplete,
...
function defaultRangeForNavigationMode(mode) {
  ...
  if (mode === 'close-coordinate') return COORDINATE_RANGE_M;
