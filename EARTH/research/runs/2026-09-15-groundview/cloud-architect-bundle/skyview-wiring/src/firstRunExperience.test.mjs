...
test('a fresh session does not receive the launcher while it is hidden', () => {
  ...
  // Hidden, not deleted. The replay hatch still opens the card.
...
test('dismissal is session-scoped; only the checkbox suppresses durably', () => {
  ...
  // The next session still does not open it. The card is hidden, not deleted.
...
test('the checkbox writes and clears durable suppression, and a storage reset undoes it', () => {
  ...
  // A cleared profile still does not open the hidden card.
  setFirstRunSuppressed(true, storage);
  assert.equal(shouldShowFirstRun({
    storage: memoryStorage(FIRST_RUN_STORAGE_KEY),
    sessionStorageRef: memoryStorage(FIRST_RUN_SESSION_KEY),
...
test('a THROWING storage getter still fails open — Safari private mode', () => {
  ...
  try {
    ...
    assert.equal(
      shouldShowFirstRun({ location: { search: '' } }),
      false,
      'a visitor whose storage throws still must not see the hidden launcher',
    );
...
test('the voice TOOL SCHEMA matches the pinned release — the mission mapping is instructions only', () => {
  ...
  assert.equal(block.length, 26223, 'serialized tool schema length drifted');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    'da86a31b4f0fca9108f9c4ffd4cc4d84224f448f34406e20fe7da77a0fa38317',
    'shipped tool schema pin, including groundview on the existing layer enums',
  );
