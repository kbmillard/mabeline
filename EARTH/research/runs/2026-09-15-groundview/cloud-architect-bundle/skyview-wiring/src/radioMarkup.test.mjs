...
test('no unchanged Realtime tool definition drifts silently', () => {
  ...
  // `bing-road` stack leaving `set_map_stack`'s enum, and GroundView layer
  // enums on set_layer_visibility / show_data_layers_menu / get_entity_context
  // are the known schema changes. Everything else must be byte-identical.
  //
  ...
  const digest = createHash('sha256')
    ...
    .digest('hex')
    .slice(0, 16);
  assert.equal(digest, '6977ee681b5cb672', 'an unchanged Realtime tool definition drifted');
});
