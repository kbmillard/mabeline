...
| **GroundView RF** (local replay/fixtures) | GROUNDVIEW layer: receivers, truck passes, matches, tracks | Local/synthetic rtl_433 NDJSON under `.gev-cache/groundview/`; empirical RF remains FIELD_VALIDATION_REQUIRED | "GROUNDVIEW: local RF replay/fixtures" |

### Notes on the live sources

- **GroundView.** Software-complete on fixtures and replay. Cesium is a projection of `/api/groundview` state. Passes are drawn at receiver coverage, not as vehicle GPS. Matches and tracks are INFERRED relations. Enabling GROUNDVIEW locally: `npm run dev` in SkyView, turn on GROUNDVIEW under The Mabeline Project. Replay: `node scripts/groundview-replay.mjs`.
